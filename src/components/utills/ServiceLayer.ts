...
import { createGet, createGetHeaders, createGetParams, createPost } from "./WebClient";
...
export const chatService = async (messageID: string | null, message: string): Promise<Record<string, unknown>> => {
...
export const getChatsService = async (messageID: string): Promise<Record<string, unknown>> => {
    ...
    try {
        ...
        const resp = await createGetParams('/api/v1/chat', params, headers);
        if (resp.status !== config.OK_STATUS) {
            console.log("ChatService not working! due to " + resp.data)
        }
        return resp.data
